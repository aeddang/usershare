...
public class TFLitePoseEstimator {
    ...
    public TFLitePoseEstimator(int height, int width, int outputStride, int numKeyPoints, int kLocalMaximumRadius) {
        this.height = height;
        this.width = width;
    ...
    private boolean scoreIsMaximumInLocalWindow(int keypointId, float score, int heatmapY, int heatmapX, int localMaximumRadius, float[][][] heatmaps) {
        ...
        for (int yCurrent = yStart; yCurrent < yEnd; ++yCurrent) {
            ...
            for (int xCurrent = xStart; xCurrent < xEnd; ++xCurrent) {
                if (heatmaps[yCurrent][xCurrent][keypointId] > score) {
                    localMaximum = false;
                    break;
                }
            }
        ...
        for (int heatmapY = 0; heatmapY < height_map; ++heatmapY) {
            for (int heatmapX = 0; heatmapX < width_map; ++heatmapX) {
                for (int keypointId = 0; keypointId < numKeyPoints; ++keypointId) {
                    float score = heatmaps[heatmapY][heatmapX][keypointId];
    ...
    private PoseTypes.Vector2DF getPafmapsPoint(int y, int x, int midIndex, float[][][] pafmaps) {
        float paf_x = pafmaps[y][x][midIndex*2];
        float paf_y = pafmaps[y][x][midIndex*2+1];
    ...
    private PoseTypes.Vector2DF getShortOffsetPoint(int y, int x, int keypoint, float[][][] offsets) {
        return types.new Vector2DF(offsets[y][x][keypoint*2 + 1], offsets[y][x][keypoint*2]);
    }
    ...
    private PoseTypes.Vector2DF getMidOffsetPoint(int y, int x, int midIndex, float[][][] offsets) {
        ...
        return types.new Vector2DF(offsets[y][x][midIndex*2+1], offsets[y][x][midIndex*2]);
    }
                    ...
                    for (int yCurrent = yStart; yCurrent < yEnd; ++yCurrent) {
                        for (int xCurrent = xStart; xCurrent < xEnd; ++xCurrent) {
                            ...
                            if (cellIdx[0] == heatmapY && cellIdx[1] == heatmapX)
                                vote += heatmaps[yCurrent][xCurrent][keypointId];
                        }
                    }

                    output[heatmapY][heatmapX][keypointId] = vote / (float) ((votingRadius + 2) * (votingRadius + 2)) * score;
                }
            }
        }
        ...
        for (int heatmapY = 0; heatmapY < height_map; ++heatmapY) {
            for (int heatmapX = 0; heatmapX < width_map; ++heatmapX) {
                for (int keypointId = 0; keypointId < numKeyPoints; ++keypointId) {
                    ...
                    int cell_center_x = (int)(width_pixel * heatmapX + cell_half_size_x + shortOffsets[heatmapY][heatmapX][keypointId*2]);
                    int cell_center_y = (int)(height_pixel * heatmapY + cell_half_size_y + shortOffsets[heatmapY][heatmapX][keypointId*2+1]);

                    int[] cellIdx = {limit((int) (cell_center_y / height_pixel), 0, height_map - 1), limit((int) (cell_center_x / width_pixel), 0, width_map - 1)};

                    float score = heatmaps[heatmapY][heatmapX][keypointId];
                    output[cellIdx[0]][cellIdx[1]][keypointId] += (float)(score / kp_maps_r2);
                }
            }
        }
    ...
    public ArrayList<float[][]> decodeOpenPoses(float[][][] _heatmaps, float[][][] _pafmaps, float[][][] shortOffsets, int maxPoseDetections, float scoreThreshold, int nmsRadius, int kpRadius, int minKeyPoints) {
    ...
    public ArrayList<float[][]> decodePoses(float[][][] _heatmaps, float[][][] shortOffsets, float[][][] midOffsets, int maxPoseDetections, float scoreThreshold, int nmsRadius, int kpRadius, int minKeyPoints) {
        ArrayList<float[][]> poses = new ArrayList<>();

        //float[] heatmaps = computeHeatmaps(_heatmaps, shortOffsets, scoreThreshold, kLocalMaximumRadius);
        //float[][][] heatmaps = Utils.minMaxFilter3D(_heatmaps, popTempHeatmaps(), width_map, height_map, numKeyPoints, 3, true, true);
        //float[] heatmaps = Utils.minMaxFilter(houghVote(_heatmaps, shortOffsets), popTempHeatmaps(), width_map, height_map, numKeyPoints, 3, true, true);
